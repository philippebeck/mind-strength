<template>
  <figure class="slider">
    <ul class="controls">
      ...
      <li>
        <button v-if="random === true" @click="checkRandom()" id="slider-random" title="Normal">
      ...
      <li>
        <button v-if="auto === true" @click="checkAuto()" id="slider-auto" title="Pause">
...
export default {
  ...
  methods: {
     ...
     */
    runSlider() {
      this.autoState ? (this.intervalId = window.setInterval(this.goNext, this.delay)) : this.goNext()
    }
  }
}
</script>
